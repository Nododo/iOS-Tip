...
/**
 ...
 对视频进行 H264 编码(硬编码)，对音频进行 AAC 编码，在 ios 中分别有已经封装好的编码库来实现对音视频的编码。
 对编码后的音、视频数据进行组装封包；PES流(Packet Elementary Stream): 也叫打包的基本码流, 是将基本的码流ES流根据需要分成长度不等的数据包, 并加上包头就形成了打包的基本码流PES流.
 建立 RTMP 连接并上推到服务端。
...
/**
 GOP ( Group of Pictures ) 是一组连续的画面，由一张 I 帧 (关键帧)和数张 B / P 帧组成，是视频图像编码器和解码器存取的基本单位，它的排列顺序将会一直重复到影像结束。
 */
...
//CDN即Content Delivery Network （内容分发网络)
...
//BGP中转架构-最短传输路径
/**
 BGP的技术原理往简单的说就是允许同一IP在不同网络中广播不同的路由信息，效果就是同一个IP。
 直播加速是流式的，并且当要做到低延迟的时候，中间的缓存要尽可能少。 BGP相当于给跨网的用户就近搭建了一坐桥梁，不必绕远路，延时和稳定性都大大提高了。
 */
